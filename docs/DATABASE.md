...
## Database Constraints (Added 2026-02-01)

### trade_entries

```sql
-- Direction must be valid
CONSTRAINT trade_entries_direction_check 
CHECK (direction IN ('LONG', 'SHORT', 'long', 'short'))

-- Source must be valid  
CONSTRAINT trade_entries_source_check
CHECK (source IS NULL OR source IN ('binance', 'manual', 'paper', 'import'))

-- Unique binance_trade_id per user (prevents duplicate syncs)
UNIQUE INDEX idx_trade_entries_binance_trade_per_user 
ON (user_id, binance_trade_id) WHERE binance_trade_id IS NOT NULL
```
