-- Create function for server-side trade stats aggregation
-- This returns total P&L, win rate, and trade count directly from DB
-- Avoids client-side calculation on paginated data
...
-- Add session column to trade_entries for server-side filtering
-- This will be populated from trade_date timezone calculation
ALTER TABLE public.trade_entries 
ADD COLUMN IF NOT EXISTS session text;

-- Create index for session filtering
CREATE INDEX IF NOT EXISTS idx_trade_entries_session ON public.trade_entries(session);

-- Create function to determine session from timestamp
CREATE OR REPLACE FUNCTION public.get_trading_session(trade_time timestamptz)
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
  hour_utc integer;
BEGIN
  hour_utc := EXTRACT(HOUR FROM trade_time AT TIME ZONE 'UTC');
  
  -- Sydney: 21:00-06:00 UTC
  IF hour_utc >= 21 OR hour_utc < 6 THEN
    RETURN 'sydney';
  -- Tokyo: 00:00-09:00 UTC  
  ELSIF hour_utc >= 0 AND hour_utc < 9 THEN
    RETURN 'tokyo';
  -- London: 07:00-16:00 UTC
  ELSIF hour_utc >= 7 AND hour_utc < 16 THEN
    RETURN 'london';
  -- New York: 12:00-21:00 UTC
  ELSIF hour_utc >= 12 AND hour_utc < 21 THEN
    RETURN 'new_york';
  ELSE
    RETURN 'other';
  END IF;
...
-- Update existing trades with session value
UPDATE public.trade_entries
SET session = public.get_trading_session(trade_date)
WHERE session IS NULL;

-- Create trigger to auto-populate session on insert/update
CREATE OR REPLACE FUNCTION public.set_trade_session()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  NEW.session := public.get_trading_session(NEW.trade_date);
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trigger_set_trade_session ON public.trade_entries;
CREATE TRIGGER trigger_set_trade_session
  BEFORE INSERT OR UPDATE OF trade_date ON public.trade_entries
  FOR EACH ROW
  EXECUTE FUNCTION public.set_trade_session();